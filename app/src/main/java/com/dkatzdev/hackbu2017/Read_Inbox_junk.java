...
public class Read_Inbox_junk extends Activity{

    private static final String INBOX_URI = "content://sms/inbox";
    private static Read_Inbox_junk activity;
    private TwoLineArrayAdapter<String> smsList;
    private ListView mListView;
    Vector<Item> items = new Vector<Item>();
    Item[] itemArray;
    private ArrayAdapter<String> adapter;
    public static Read_Inbox_junk instance() {
        return activity;
    }
    @Override
    ...
    private OnItemClickListener MyItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int pos, long id) {
            try {
                Intent myIntent = new Intent(Read_Inbox_junk.this, sendSMS.class);
                myIntent.putExtra(Intent.EXTRA_TEXT, itemArray[pos].getPhoneNumber());
                Read_Inbox_junk.this.startActivity(myIntent);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
